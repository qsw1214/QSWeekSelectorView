...
@protocol QSWeekSelectorViewDelegate;
 ...
 */
@interface QSWeekSelectorView : UIView

/**
 The delegate conforming to the `ASWeekSelectorViewDelegate` delegate.
 */
@property (nonatomic, weak) id<QSWeekSelectorViewDelegate> delegate;
...
@protocol QSWeekSelectorViewDelegate <NSObject>
...
/**
 ...
 @param date         The selected date.
 */
- (void)weekSelector:(QSWeekSelectorView *)weekSelector didSelectDate:(NSDate *)date;
...
/**
 ...
 @param weekSelector The week selector that the user interacted with.
 */
- (void)weekSelectorDidSwipe:(QSWeekSelectorView *)weekSelector;
